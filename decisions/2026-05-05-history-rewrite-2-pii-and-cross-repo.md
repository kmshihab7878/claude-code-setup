...
1. **Mailmap** — rewrites every commit's author/committer to:
   `<contributor> <<contributor>@users.noreply.github.com>` (GitHub-issued privacy-preserving address).
...
5. **Local repo git config** updated to use the noreply email going forward (`git config --local user.email <contributor>@users.noreply.github.com`). Global git config NOT touched — that's an operator decision per repo.
...
- All commit authors now appear as `<contributor> <<contributor>@users.noreply.github.com>`
...
| Operator's GLOBAL git config still has personal Gmail | Operator-controlled | Optional: `git config --global user.email <contributor>@users.noreply.github.com` to default ALL repos to the noreply email |
