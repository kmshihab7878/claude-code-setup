...
## HARD GATE: Design Approval Required

Before ANY implementation work based on brainstorm output:

1. Present the requirements specification to the user
2. Ask explicitly: **"Do you approve this specification for implementation?"**
3. Wait for the user's explicit approval (yes, approved, go ahead, etc.)
4. **NEVER** begin coding from an unapproved brainstorm — not even "just a prototype"

If the user wants changes, iterate on the spec first. Only proceed to `/sc:design` or `/sc:implement` after explicit approval.
