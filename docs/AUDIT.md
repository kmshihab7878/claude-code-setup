...
4. **The ecosystem has real extraction value and it is visible.** Nine clusters are already self-contained enough to ship as standalone artifacts: Elite Ops layer (12 files), self-evolution layer (~25 files), `/market` suite (~22 files), CoreMind-Core governance (coremind-core + governance-gate + REGISTRY), Hue meta-skill (SKILL + 19 brand demos — biggest concrete demo in the repo), React-bits catalog, MCP-mastery, anti-ai-writing filter, CoreMind-sec.
...
| 2 | `agents/REGISTRY.md` | **Operational + Positioning** — central dispatch table, the CoreMind-mirrored architecture proof |
...
| 5 | `skills/coremind-core/SKILL.md` + `skills/governance-gate/SKILL.md` | **Operational + Extraction** — 10-stage pipeline + 5-layer safety gate |
...
| 8 | **"The pipeline" declared in 9 files with 10/13/15-stage variants** (plan=10, ELITE-OPS=13, ultraplan=15, coremind-core=10, ...) | Operational confusion | No single answer to "what's the pipeline?" |
...
| Pipeline authorities | plan + ultraplan + coremind-core + CLAUDE.md + ELITE-OPS-METHODOLOGY + governance-gate + operating-framework + REGISTRY + OPERATING_FRAMEWORK doc | No | `skills/coremind-core/SKILL.md` should be the single source; others link to it |
...
- Top-20 skills (coremind-core, governance-gate, operating-framework, coding-workflow, moyu, prompt-reliability-engine, test-driven-development, git-workflows, security-review, api-design-patterns, python-quality-gate, n8n, council)
...
- `agents/REGISTRY.md` + `skills/coremind-core/SKILL.md` + `skills/governance-gate/SKILL.md` — CoreMind-mirrored governance story
...
| **CoreMind-sec** | Autonomous security ecosystem | Yes | `skills/coremind-sec/` + `/coremind-sec` command |
| **CoreMind-Core governance** | 10-stage pipeline + 5-layer safety gate | Yes | `skills/{coremind-core,governance-gate}/` + `agents/REGISTRY.md` |
...
- `understand-coremind` + `understand-memory-sync` → consolidate into `understand-chat` and `understand` respectively (niche variants)
...
- `docs/CLAUDE_CODE_ARCHITECTURE.md` → replace with a pointer to CLAUDE.md's "CoreMind-Mirrored Architecture" section + REGISTRY.md. 524 lines → ~40 lines.
...
- The 5 extraction candidates (Elite Ops, self-evolution, /market, Hue, CoreMind-sec) are good candidates for Phase 1's `domains/` regrouping — but in-place is fine; what matters is making their boundaries obvious.
...
2. `agents/REGISTRY.md` + top-20 core agents + `skills/{coremind-core, governance-gate, operating-framework, elite-ops, moyu, prompt-reliability-engine, coding-workflow}/` — the governance/planning stack
