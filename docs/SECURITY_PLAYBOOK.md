...
ls -la ~/JARVIS-FRESH/.githooks/pre-commit
cat ~/JARVIS-FRESH/scripts/compass/patterns.blocklist.txt
```
...
grep -n "algorithm" ~/JARVIS-FRESH/src/jarvis/security/jwt_service.py
```
...
ls -la ~/JARVIS-FRESH/src/jarvis/security/rbac.py
```
...
ls -la ~/JARVIS-FRESH/src/jarvis/gaos/input_sanitizer.py
```
...
grep -n "ALLOW\|REVIEW\|ESCALATE\|BLOCK" ~/JARVIS-FRESH/src/jarvis/core/policy_gate.py | head -20
```
...
grep -rn "agent\.run(" ~/JARVIS-FRESH/src/ --include="*.py" | grep -v "coordinator"
# Run compass check
bash ~/JARVIS-FRESH/scripts/compass/compass_check.sh
```
...
**Verification**:
```bash
ls -la ~/JARVIS-FRESH/src/jarvis/gaos/capability_token.py
ls -la ~/JARVIS-FRESH/src/jarvis/gaos/redis_capability_token.py
```
...
**Verification**:
```bash
ls -la ~/JARVIS-FRESH/src/jarvis/security/hash_chain.py
ls -la ~/JARVIS-FRESH/src/jarvis/security/audit_logger.py
```
...
grep -rn "USER" ~/JARVIS-FRESH/docker/ --include="Dockerfile*"
```
...
git -C ~/JARVIS-FRESH ls-files | grep -i "tfstate"
```
...
**Verification**:
```bash
ls -la ~/JARVIS-FRESH/src/jarvis/gaos/output_validator.py
```
...
# 7. Run compass check
cd ~/JARVIS-FRESH && bash scripts/compass/compass_check.sh

# 8. Verify git hooks installed
ls -la ~/JARVIS-FRESH/.githooks/pre-commit
ls -la ~/JARVIS-FRESH/.githooks/pre-push

# 9. Check for direct agent.run() calls
grep -rn "agent\.run(" ~/JARVIS-FRESH/src/ --include="*.py" | grep -v "coordinator" | grep -v "test"

# 10. Verify GAOS modules present
ls ~/JARVIS-FRESH/src/jarvis/gaos/*.py

# 11. Verify security modules present
ls ~/JARVIS-FRESH/src/jarvis/security/*.py

# 12. Check for hardcoded secrets
grep -rn "changeme\|password\s*=\s*['\"]" ~/JARVIS-FRESH/src/ --include="*.py"

# --- Container Layer ---

# 13. Check Dockerfiles for non-root
grep -rn "USER" ~/JARVIS-FRESH/docker/ --include="Dockerfile*" 2>/dev/null
