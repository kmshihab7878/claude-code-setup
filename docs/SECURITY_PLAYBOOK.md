...
- CoreMind compass blocklist scans for `"changeme"` literals
...
- CoreMind: All secrets load from environment variables; missing secrets cause `RuntimeError` at startup
- CoreMind CLAUDE.md Section 1.2: "No default secrets may exist in production code"
...
**Enforcement**:
- CoreMind: `.githooks/pre-commit` runs `compass_check.sh` which scans for forbidden patterns
...
**Verification**:
```bash
# Verify CoreMind git hooks are active
...
**Enforcement**:
- CoreMind: `config/policies/` contains environment-specific policy configs
...
**Enforcement**:
- CoreMind: `src/coremind/security/jwt_service.py` — JWT creation and validation
- CoreMind: `src/coremind/security/redis_token_blacklist.py` — Token revocation via Redis
- CoreMind: `src/coremind/security/token_blacklist.py` — In-memory fallback
- WebSocket auth requires valid JWT (Section 7 of COMPASS)

**Verification**:
```bash
# Verify JWT service exists and uses proper algorithms
grep -n "algorithm" ~/your-project/src/coremind/security/jwt_service.py
```
...
**Enforcement**:
- CoreMind: `src/coremind/security/rbac.py` — Role definitions and permission checks
...
ls -la ~/your-project/src/coremind/security/rbac.py
```
...
**Enforcement**:
- CoreMind: `src/coremind/security/auth_sessions.py` — Session management
...
**Enforcement**:
- CoreMind: Distributed rate limiting via Redis
...
**Enforcement**:
- CoreMind GAOS Layer 1: `src/coremind/gaos/input_sanitizer.py`
...
ls -la ~/your-project/src/coremind/gaos/input_sanitizer.py
```
...
**Enforcement**:
- CoreMind: `src/coremind/core/policy_gate.py` — 7 constraints enforced at every gate
- PolicyGate constraints may NOT be bypassed without ESCALATE-level approval
- CoreMind CLAUDE.md Section 1.2: "Governance Cannot Be Weakened"

**Verification**:
```bash
# Verify PolicyGate implementation
grep -n "ALLOW\|REVIEW\|ESCALATE\|BLOCK" ~/your-project/src/coremind/core/policy_gate.py | head -20
```
...
- CoreMind GAOS input sanitizer detects injection patterns at runtime
...
- CoreMind: FastAPI CORS middleware with specific allowed origins
...
**Enforcement**:
- CoreMind: `contextvars` guard raises `CoordinatorSecurityError` at runtime for direct calls
...
**Enforcement**:
- CoreMind: `src/coremind/core/contracts/` — Frozen dataclasses
...
**Enforcement**:
- CoreMind: `src/coremind/gaos/capability_token.py` — Token issuance
- CoreMind: `src/coremind/gaos/redis_capability_token.py` — Redis-backed tokens
- Flow: `gaos.authorize_execution()` → capability token → `gaos.execute_with_token()`
- Tokens are revocable mid-execution

**Verification**:
```bash
ls -la ~/your-project/src/coremind/gaos/capability_token.py
ls -la ~/your-project/src/coremind/gaos/redis_capability_token.py
```
...
**Enforcement**:
- CoreMind: `src/coremind/security/hash_chain.py` — SHA-256 chained audit log
- CoreMind: `src/coremind/security/audit_logger.py` — Audit event persistence
...
**Verification**:
```bash
ls -la ~/your-project/src/coremind/security/hash_chain.py
ls -la ~/your-project/src/coremind/security/audit_logger.py
```
...
**Enforcement**:
- CoreMind: `docker/` directory contains Dockerfiles
...
**Enforcement**:
- CoreMind: `terraform/` directory contains infrastructure code
...
**Enforcement**:
- CoreMind: `config/phase_gates.yml` defines environment gates
...
- CoreMind: `tests/test_health.py` validates health endpoint behavior
...
**Enforcement**:
- CoreMind GAOS Layer 4: `src/coremind/gaos/output_validator.py`
...
**Verification**:
```bash
ls -la ~/your-project/src/coremind/gaos/output_validator.py
```
...
| Component | File | Purpose |
|-----------|------|---------|
| Input Sanitizer | `src/coremind/gaos/input_sanitizer.py` | Injection detection, content validation |
| Policy Gate | `src/coremind/core/policy_gate.py` | 4-tier authorization |
| Execution Sandbox | `src/coremind/gaos/sandbox.py` | Timeouts, resource quotas |
| Output Validator | `src/coremind/gaos/output_validator.py` | PII/secret detection, redaction |
| Rollback Engine | `src/coremind/gaos/rollback.py` | Compensation transactions |
| Capability Tokens | `src/coremind/gaos/capability_token.py` | Revocable execution grants |
| Redis Tokens | `src/coremind/gaos/redis_capability_token.py` | Distributed token management |
| Redis Quotas | `src/coremind/gaos/redis_quota.py` | Resource pool enforcement |

### CoreMind Security Layer

| Component | File | Purpose |
|-----------|------|---------|
| JWT Service | `src/coremind/security/jwt_service.py` | Token creation/validation |
| RBAC | `src/coremind/security/rbac.py` | Role-based access control |
| Hash Chain | `src/coremind/security/hash_chain.py` | Tamper-evident audit |
| Audit Logger | `src/coremind/security/audit_logger.py` | Event persistence |
| Token Blacklist | `src/coremind/security/redis_token_blacklist.py` | JWT revocation |
| Auth Sessions | `src/coremind/security/auth_sessions.py` | Session management |
| API Key Auth | `src/coremind/security/api_key_auth.py` | API key validation |
...
ls ~/your-project/src/coremind/gaos/*.py

# 11. Verify security modules present
ls ~/your-project/src/coremind/security/*.py
