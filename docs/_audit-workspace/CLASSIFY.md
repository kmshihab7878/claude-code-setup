...
- **Process skills**: `redacted-private-skill`, `project-template`, `using-khaledpowers`, `quality-manager-qmr`, `para-memory-files`, `recipe`, `agent-evaluation`, `anti-ai-writing`, `confidence-check` (has .py + .ts), `context-budget-audit` — SUPPORTING.
