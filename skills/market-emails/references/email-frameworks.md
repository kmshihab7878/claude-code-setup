# Email frameworks (Phase 2 detail)

Email philosophy (one-email-one-job), three structure frameworks (Value Before Ask, Story-Driven, Problem-Agitate-Solution), 10-row subject-line-formula table with rules, and the full send-cadence matrix per sequence type + best-send-time benchmarks. SKILL.md keeps the framework names and key rules; this file holds the full templates and tables.
